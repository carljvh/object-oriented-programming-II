...
//						+ "\tBlack: " + this.blackScore);
				...
				support.firePropertyChange("Score", false, true);
			}
	...
	public void gameUpdate(final int lastKey) throws GameOverException {
		...
		// Notify observers
		support.firePropertyChange("Gameboard", false, true);
	}
	...
	public GameTile getGameboardState(final int x, final int y) {
		PieceColor posColor = this.board[x][y];
		GameTile posTile;
		
		if (posColor == PieceColor.EMPTY) {
			posTile = blankTile;
		} else {
			posTile = (posColor == PieceColor.BLACK ? blackGridTile : whiteGridTile);
		}
		
		if (this.cursorPos.getX() == x && this.cursorPos.getY() == y) {
			if (canTurn(this.turn, this.cursorPos)) {
				if (this.turn == Turn.BLACK) {
					return new CompositeTile(posTile, cursorBlackTile);
				} else {
					return new CompositeTile(posTile, cursorWhiteTile);
				}
			} else {
				return new CompositeTile(posTile, cursorRedTile);
			}
		}
		return posTile;
	}
